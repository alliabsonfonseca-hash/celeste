...
def main():
    ...
    with st.form("simulador_form"):
        ...
        with col1:
            valor_total_str = st.text_input("Valor Total do Imóvel (R$)", key="valor_total_str", placeholder="Ex: 485.458,00")
            entrada_str = st.text_input("Entrada (R$)", key="entrada_str", placeholder="Ex: 45.000,00")
        ...
        if st.session_state.get("modalidade") == "mensal + balão":
            st.subheader("Configuração dos Balões")
            ...
            with b_col2:
                if st.session_state.get('agendamento_baloes') == "Personalizado (Mês a Mês)":
                ...
                elif st.session_state.get('agendamento_baloes') == "A partir do 1º Vencimento":
                    max_p = st.session_state.get('qtd_parcelas') or 1
                    st.number_input("Mês de Vencimento do 1º Balão", min_value=1, max_value=max_p, value=min(12, max_p), step=1, key="mes_primeiro_balao")
    ...
    if submitted:
        try:
            ...
            qtd_parcelas_val = qtd_parcelas or 0
            
            taxa_mensal_para_calculo = taxa_mensal if not (1 <= qtd_parcelas_val <= 36 and modalidade == 'mensal') else 0.0
            ...
            datas_p = [ajustar_data_vencimento(data_entrada, "mensal", i) for i in range(1, qtd_parcelas_val + 1)]
            datas_b = []
            if qtd_baloes > 0:
            ...
            cronograma = gerar_cronograma(valor_financiado, valor_parcela_final, valor_balao_final, qtd_parcelas_val, qtd_baloes, modalidade, tipo_balao, data_entrada, taxas, agendamento_baloes, meses_baloes, mes_primeiro_balao)
