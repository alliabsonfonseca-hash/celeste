...
def main():
    ...
    if logo:
        ...
        col2.title("**Bem-vindo ao Simulador da JMD HAMOA**")
    else:
    ...
    with st.form("simulador_form"):
        ...
        with col1:
            ...
            taxa_mensal = st.number_input("Taxa de Juros Mensal (%)", value=st.session_state.taxa_mensal, step=0.01, format="%.2f", key="taxa_mensal_input", help="Use vírgula para decimais (ex: 0,79) e não use pontos.")
            modalidade = st.selectbox("Modalidade de Pagamento", ["mensal", "mensal + balão", "só balão anual", "só balão semestral"], key="modalidade")
            
            tipo_balao, agendamento_baloes = "anual", "Padrão"
            if modalidade == "mensal + balão":
                ...
                max_p = st.session_state.get('qtd_parcelas', 360)
                if not isinstance(max_p, int) or max_p <= 0: max_p = 1
        ...
        with col2:
            ...
            valor_parcela = st.number_input("Valor da Parcela (R$)", help="Deixe 0 para cálculo automático.", min_value=0.0, step=100.0, format="%.2f", key="valor_parcela")
            valor_balao = 0.0
            if "balão" in modalidade:
                st.write(f"Quantidade de Balões: **{atualizar_baloes(modalidade, qtd_parcelas, tipo_balao if 'tipo_balao' in locals() else None)}**")
                valor_balao = st.number_input("Valor do Balão (R$)", help="Deixe 0 para cálculo automático.", min_value=0.0, step=1000.0, format="%.2f", key="valor_balao")
    ...
    if submitted:
        try:
            ...
            valor_parcela_final, valor_balao_final, v_primeira_p, v_primeiro_b = 0.0, 0.0, None, None
            ...
            temp_cronograma = gerar_cronograma(valor_financiado, valor_parcela_final, valor_balao_final, qtd_parcelas, qtd_baloes, modalidade, tipo_balao, data_entrada, taxas, None, None, agendamento_baloes, meses_baloes, mes_primeiro_balao)
            if temp_cronograma:
                total_vp_calc = temp_cronograma[-1]['Valor_Presente']
                diferenca = round(valor_financiado - total_vp_calc, 2)
                if qtd_parcelas > 0: v_primeira_p = valor_parcela_final + diferenca
                elif qtd_baloes > 0: v_primeiro_b = valor_balao_final + diferenca
            
            cronograma = gerar_cronograma(valor_financiado, valor_parcela_final, valor_balao_final, qtd_parcelas, qtd_baloes, modalidade, tipo_balao, data_entrada, taxas, v_primeira_p, v_primeiro_b, agendamento_baloes, meses_baloes, mes_primeiro_balao)
            ...
            if valor_parcela_final > 0: c3.metric("Valor da Parcela", formatar_moeda(v_primeira_p if v_primeira_p is not None else valor_parcela_final))
            if valor_balao_final > 0: c4.metric("Valor do Balão", formatar_moeda(v_primeiro_b if v_primeiro_b is not None else valor_balao_final))
